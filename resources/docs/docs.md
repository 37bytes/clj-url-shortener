...
The `home-routes` handler in the `clj-url-shortener.routes.home` namespace
...
The `home-page` function will in turn call the `clj-url-shortener.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `clj-url-shortener.handler` namespace:
...
Request middleware functions are located under the `clj-url-shortener.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clj-url-shortener.dev-middleware` namespace found in
